...
class Utils:
    ...
    #         logger.info("\n✅ Excel Comparison Results:\n" + "\n".join(table_rows))
    @keyword
    def compare_excel_files(self, expected_path, actual_path, sheet_name=None, ignore_case=False, ignore_columns=None):
        """
        Compare two Excel files and return detailed mismatch info instead of failing immediately.
        """
        actual_data, actual_columns = self._extract_excel_data_with_columns(actual_path, sheet_name, ignore_case)
        expected_data, expected_columns = self._extract_excel_data_with_columns(expected_path, sheet_name, ignore_case)
        ...
        if actual_row_count != expected_row_count:
            error_msg = f"❌ Row count mismatch: Actual={actual_row_count}, Expected={expected_row_count}"
            logger.error(error_msg)
        ...
        if len(filtered_actual_columns) != len(filtered_expected_columns):
            error_msg = (
                f"❌ Column count mismatch (after ignoring specified columns):\n"
                f"Actual columns: {filtered_actual_columns}\nExpected columns: {filtered_expected_columns}"
            )
            logger.error(error_msg)
            return False, error_msg
        ...
        for col_idx, col_name in enumerate(actual_columns):
            ...
            missing = [val for val in expected_col_data if val not in actual_col_data]
            unexpected = [val for val in actual_col_data if val not in expected_col_data]

            if missing or unexpected:
                has_mismatch = True
                mismatch_details.append(f"Column '{col_name}': Missing={missing}, Unexpected={unexpected}")

            comparison_results[col_name] = {
                'Expected Data': missing,
                'Actual Data': unexpected
            }

        if has_mismatch:
            mismatch_text = "\n".join(mismatch_details)
            logger.error(f"\n❌ Excel Mismatches Found:\n{mismatch_text}")
            return False, mismatch_text
        else:
            logger.info("✅ Excel files match perfectly.")
            return True, "All columns match successfully."
